...
switch (state)
{
	...
	state_set_sprite(enemy_hill_orc_stand, 1, 0);
	...
	case "Chase" :
	#region Chase state
		state_set_sprite(enemy_hill_orc_move, 1, 0);
		if !instance_exists(oPlayer) break;
	...
	if !instance_exists(oPlayer) break;
		if act_switch == false
		{
			state_set_sprite(enemy_hill_orc_attack, 1, 0);
			...
			{
				creat_hitbox(x, y, self, enemy_hill_orc_attack_hitbox , knockback_power, 3, attack_power, image_xscale);
			}
		}
	...
	case "Knockback" :
		#region Knockback_state
			...
			{audio_play_sound(SE_enemy_knockback01, 1, false);}
			moving_speed = 0;
	...
	#region
		...
		if animation_end()
		{instance_destroy();instance_create_depth(x, y-22, 0, dead_item);}
